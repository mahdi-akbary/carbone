...
var carbone = {
   ...
   *                           lang         : set default lang of API-REPORT-GENERATOR, can be overwrite in API-REPORT-GENERATOR.render options
   *                           translations : overwrite API-REPORT-GENERATOR translations object
  ...
  /**
   * Add a template in API-REPORT-GENERATOR datastore (template path)
  ...
  /**
   * Remove a template from the API-REPORT-GENERATOR datastore (template path)
...
function convert (data, convertTo, options, callback) {
  ...
  if (format.document[convertTo] === undefined) {
    return callback('Format "'+ convertTo +'" not accepted. Accepted formats: '+Object.keys(format.document).join(', '));
  }
