...
var builder = {
  ...
  assembleXmlParts : function (arrayOfStringPart, sortDepth) {
    ...
    for (var i = 0; i < arrayOfStringPart.length; i++) {
      ...
      var nextStr = i < arrayOfStringPart.length ? arrayOfStringPart[i + 1] : null;
      if (nextStr && !isNaN(nextStr.str)) {
        var temp = _part.str.replace('<table:table-cell', '<table:table-cell office:value=\"' + nextStr.str + '\" ')
        temp = temp.replace('string', 'float');
        temp = temp.replace('string', 'float');
        _part.str = temp;
        _res += temp;
      } else {
